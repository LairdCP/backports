...
static int nl802154_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	case NL802154_SCAN_PASSIVE:
		...
		break;
	default:
		NL_SET_ERR_MSG_FMT(genl_info_extack(info),
...
static int nl802154_add_llsec_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL802154_ATTR_SEC_KEY] ||
	    nla_parse_nested_deprecated(attrs, NL802154_KEY_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_KEY], nl802154_key_policy, genl_info_extack(info)))
		return -EINVAL;
...
static int nl802154_del_llsec_dev(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL802154_ATTR_SEC_DEVICE] ||
	    nla_parse_nested_deprecated(attrs, NL802154_DEV_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_DEVICE], nl802154_dev_policy, genl_info_extack(info)))
		return -EINVAL;
...
static int nl802154_del_llsec_devkey(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL802154_ATTR_SEC_DEVKEY] ||
	    nla_parse_nested_deprecated(attrs, NL802154_DEVKEY_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_DEVKEY], nl802154_devkey_policy, genl_info_extack(info)))
		return -EINVAL;
...
static int nl802154_pre_doit(__genl_const struct genl_split_ops *ops,
			     struct sk_buff *skb,
			     struct genl_info *info)
{
	struct cfg802154_registered_device *rdev;
...
static void nl802154_post_doit(__genl_const struct genl_split_ops *ops,
			       struct sk_buff *skb,
			       struct genl_info *info)
{
	if (info->user_ptr[1]) {
