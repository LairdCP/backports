					     ...
					     t_u8 wait_option)
{
	...
	/* make copy of file path */
	strncpy(last_rg_file_path, file_path, sizeof(file_path));
	...
	if ((handle->params.cntry_txpwr == CNTRY_RGPOWER_MODE) &&
	    (ret == MLAN_STATUS_FILE_ERR)) {
		PRINTM(MMSG, "wlan: %s load failed as file not exist!!\n",
		       file_path);
	...
	while (!list_empty(&handle->evt_queue)) {
		...
		case WOAL_EVENT_RGPWR_KEY_MISMATCH:
			if (handle->country_code[0] == '0' &&
				...
				PRINTM(MMSG,
				       "wlan: %s load failed due to key mismatch!!\n",
				       last_rg_file_path);
...
#define NAPI_BUDGET 64
	...
	woal_update_firmware_name(handle);
#ifdef ANDROID_KERNEL
	lrd_wakeup_source_init(&handle->ws, "mwlan");
#endif
...
#ifdef ANDROID_KERNEL
	lrd_wakeup_source_trash(&handle->ws);
#endif
	/* Unregister device */
