...
void bp_woal_tx_timeout(struct net_device *dev);
...
#else
	.ndo_tx_timeout = bp_woal_tx_timeout,
#endif
...
EXPORT_SYMBOL_GPL(bp_woal_tx_timeout);
#endif
			   ...
			   t_u16 card_type)
{
	...
	handle->ws = wakeup_source_register(dev, "mwlan");
#else
	handle->ws = wakeup_source_register("mwlan");
#endif
#endif
...
#ifdef ANDROID_KERNEL
	wakeup_source_unregister(handle->ws);
#endif
	/* Unregister device */
