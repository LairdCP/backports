				   ...
				   struct ieee80211_bss_conf *bss_conf)
{
	...
	if (mwl_vif->beacon_info.ie_rsn48_len > LRD_MAX_RSNIE_DATA) {
		wiphy_err(priv->hw->wiphy,
	...
	if (mwl_vif->beacon_info.ie_rsn_len) {
		...
		if ( mwl_vif->beacon_info.ie_rsn48_len + mwl_vif->beacon_info.ie_rsn_len <= LRD_MAX_RSNIE_DATA){
			      ...
			      struct ieee80211_sta *sta)
{
	...
	if (hw->conf.chandef.chan->band == NL80211_BAND_2GHZ) {
		 ...
		 */
		legacy_rates = sta->supp_rates[NL80211_BAND_2GHZ] & 0xF;

		/* shift OFDM rates by 1 */
		legacy_rates |= (sta->supp_rates[NL80211_BAND_2GHZ] & 0xFF0) << 1;
	} else {
		/*
		 ...
		 * Shift by 5
		 */
		legacy_rates = sta->supp_rates[NL80211_BAND_5GHZ] << 5;
	}
